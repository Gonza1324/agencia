alter table public.daily_settlements
  add column overpayment_credit_amount numeric(14, 2) not null default 0,
  add constraint daily_settlements_overpayment_credit_amount_nonnegative
    check (overpayment_credit_amount >= 0);

comment on column public.daily_settlements.overpayment_credit_amount is
  'Saldo a favor generado por recibir más dinero que el importe esperado.';
...
begin
  if new.sales_amount is null then
    ...
    new.overpayment_credit_amount := 0;
    new.debt_amount := 0;
  ...
  new.overpayment_credit_amount := greatest(
...
create or replace function public.create_overpayment_credit_account_movement()
returns trigger
language plpgsql
security invoker
set search_path = ''
as $$
begin
  if new.overpayment_credit_amount <= 0 then
    return new;
  end if;
  ...
  values (
    ...
    'overpayment_credit',
    'credit',
    new.overpayment_credit_amount,
    new.id,
    'Saldo a favor por pago superior al importe esperado',
    new.created_by
  );
...
revoke all on function public.create_overpayment_credit_account_movement()
  from public, anon, authenticated;

create trigger daily_settlements_create_overpayment_credit
after insert on public.daily_settlements
for each row execute function public.create_overpayment_credit_account_movement();
...
begin
  ...
  if corrected_definition = current_definition then
    raise exception 'No se encontró la validación de excedente a actualizar';
  end if;
